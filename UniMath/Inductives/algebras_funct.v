Require Export UniMath.Inductives.famisfunctor.
...
(*Section Algebras.
  ...
  (*Variable A: Fam I.
  Variable F : functor (Fam I) (Fam I).
  Eval compute in (F A).
  Eval compute in A.*)

  Definition algebra_structure (F : functor (Fam I) (Fam I))
             (A : ob (Fam I)) :=
    Fam I ⟦ F A , A ⟧.

  Definition algebra (F : functor (Fam I) (Fam I)) : UU :=
    ∑ carrier, algebra_structure F carrier.

  Definition algebra_to_type {F : functor (Fam I) (Fam I)} :
    algebra F -> I -> UU :=
    pr1.
  Coercion algebra_to_type : algebra >-> Funclass.

  Definition algebra_to_algebra_str {F : functor (Fam I) (Fam I)}
             (A : algebra F) :
  ...
  Definition algebra_str_morphism
             (F : functor (Fam I) (Fam I)) (A B : algebra F)
             (h : Fam I ⟦ A , B ⟧) : UU :=
    ∏ i x, h i (A.2 i x) = B.2 i (#F h i x).

  Definition algebra_morphism {F : functor (Fam I) (Fam I)}
             (A B : algebra F) : UU :=
    ∑ map, algebra_str_morphism F A B map.

  Definition algebra_morphism_to_function {F : functor (Fam I) (Fam I)}
             {A B : algebra F}
             (f : algebra_morphism A B) : Fam I ⟦ A , B ⟧ :=
    pr1 f.
  Coercion algebra_morphism_to_function : algebra_morphism >-> precategory_morphisms.


  Definition algebra_morphism_identity {F : functor (Fam I) (Fam I)}
             (A : algebra F) :
  ...
  Definition algebra_morphism_composition {F : functor (Fam I) (Fam I)}
             {A B C : algebra F}
  ...
  Proof.
    ...
    cbn.
    unfold funcomp.
    rewrite <- g.2. rewrite <- f.2.
    reflexivity.
  Defined.

  (* initial algebras *)
  Definition is_initial {F : functor (Fam I) (Fam I)} (A : algebra F) : UU :=
    ∏ B : algebra F, iscontr (algebra_morphism A B).

  Definition initiality_morphism {F : functor (Fam I) (Fam I)} (A : algebra F)
             (d : is_initial A) (B : algebra F) : algebra_morphism A B :=
    pr1 (d B).

  Definition uniqueness_morphism {F : functor (Fam I) (Fam I)} (A : algebra F)
             (d : is_initial A) (B : algebra F) (m m' : algebra_morphism A B) :
    m = m'.
  Proof.
    intros. apply proofirrelevancecontr. apply d.
  Qed.

  Definition is_prop_is_initial {F : functor (Fam I) (Fam I)}
             (A : algebra F) : isaprop (is_initial A).
  Proof.
...
Section CoAlgebras.
  ...
  Definition coalgebra_structure (F : functor (Fam I) (Fam I)) (A : Fam I) :=
    A --> F A.
